...
技术说明：将 personal/data/ 原始资料提炼为 personal/knowledge/ 结构化知识，严格按 0→8 步顺序执行。

## Quick Reference

| 步骤 | 名称 | 操作 | 失败处理 |
|:---:|------|------|------|
| 0 | 预检 | 扫描 queue+personal/data/inbox/converted/+personal/data/inbox/ → 敏感扫描 → Lint 过期检查 | 失效标记[!]跳过 |
| 1 | 读取 | 逐条读 personal/data/ 原始资料，提取核心主张 | — |
| 1.5 | 本地解析 | personal/raw/→拆分→写入 personal/data/sessions/ | 移至 _failed/ |
...
| 6 | 更新索引 | personal/index.md 追加条目 | 冲突→临时文件 |
...
## 降级
SKILL 加载失败时，直接读取：`framework/ops/rules/Ingest完整流程.md`
