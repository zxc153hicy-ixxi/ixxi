...
- ✅ 已确认（2026-08-16）：G2/G15 加「沉积前判 scope」——不涉隐私 AND 绝大部分人有用 → framework，否则 personal（写入时分类，非事后盘点）
  - 确认方式：用户对话回复「那么做吧」（在选定口径A「绝大部分 ixxi 用户」+ 写入时分类机制后）
- ✅ 已确认（2026-08-16）：G21「待办必登记」backlog 路径改「维护者 backlog（未落地清单，personal 侧）」——6 个维护文档移出 framework 后的路径修正，避免失效路径 + 不硬编码 personal 项目名
  - 确认方式：用户对话回复「确认修改」（四项确认清单）
